					...
					switch(ctype)
					{
						...
						case Types.TIMESTAMP:
						{
							...
							if(ctime!=null)
							{
								...
								row.setAttr(meta.getColumnLabel(i + 1).toLowerCase(), StringToolKit.formatText(ctimestr));
							}
							else
							{
								row.setAttr(meta.getColumnLabel(i + 1).toLowerCase(), new String());
							}
							break;
						}
						default:
						{
							row.setAttr(meta.getColumnLabel(i + 1).toLowerCase(), StringToolKit.formatText(rs.getString(i + 1)));
						}
					}
	...
	public static List packEmptyObjData(ResultSet rs) throws SQLException
	{
		...
		if (rs != null)
		{
			...
			if(empty==0)
			{
				...
				for (int i = 0; i < columns; i++)
				{
					row.setAttr(meta.getColumnLabel(i + 1).toLowerCase(), "");
				}
				data.add(row);
			}
		}
		...
		if (rs != null)
		{
			...
			while (rs.next())
			{
				if ((current >= startIndex) && (current < endIndex))
				{
					...
					for (int i = 0; i < columns; i++)
					{
						row.setAttr(meta.getColumnLabel(i + 1).toLowerCase(), rs.getString(i + 1));
					}
					data.add(row);
				}
...
//				}
	...
	// ��ݴ��
	public static List cpackObjData(ResultSet rs, int startIndex, int endIndex) throws SQLException
	{
		List data = new ArrayList();
		if (rs != null)
		{
			...
			for(int i=0;i<columns;i++)
			{
				colnames[i] = meta.getColumnLabel(i + 1).toLowerCase();			
			}
			int current = 0;
