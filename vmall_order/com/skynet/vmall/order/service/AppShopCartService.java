...
@InjectName("appshopcartService")
@IocBean(args =
{ "refer:dao" })
public class AppShopCartService extends SkynetDaoService
{
	public AppShopCartService()
	{
		super();
		// TODO Auto-generated constructor stub
	}

	public AppShopCartService(Dao dao)
	{
		super(dao);
		// TODO Auto-generated constructor stub
	}
	...
	public Map addtocart(DynamicObject form, DynamicObject login_token) throws Exception
	{
		...
		if (hasnums > 0)
		{
			...
			{
				DynamicObject goodsprice = appgoodsService.getPrice(new DynamicObject(new String[]
	...
	protected String makeorder(String dealid, String batchno, List<DynamicObject> shopcartgoodses, DynamicObject login_token) throws Exception
	{
		...
		for (int i = 0; i < shopcartgoodses.size(); i++)
		{
			...
			List<DynamicObject> supmembers = appmemberService.findsupmembers(userid, AppMemberService.level_rebate);
	...
	// 返回真为超过限购数量
	// public int check_shopcart_event_buyednums(String userid, String goodsid,
	// String eventitemgoodsid) throws Exception
	// {
	// EventItemGoods eventitemgoods = sdao().fetch(EventItemGoods.class,
	// eventitemgoodsid);
	//
	// StringBuffer sql = new StringBuffer();
	...
	//
	// int nums_buyed =
	// Types.parseInt(sdao().queryForMap(sql.toString()).getFormatAttr("nums"),
	// 0);
